...
def read_binary_file(filename, L1, L2, L3, inputs, num_buckets=8):
    with open(filename, "rb") as f:
        ...
        data["l0w"] = list(
            struct.unpack("<" + "h" * (inputs * L1), f.read(inputs * L1 * 2))
        )

        # Read pst (32-bit) - 720896 values (inputs * 8)
        data["pst"] = list(
            struct.unpack(
                "<" + "i" * (inputs * num_buckets), f.read(inputs * num_buckets * 4)
            )
        )
        ...
        data["l1b"] = list(
            struct.unpack(
                "<" + "i" * ((L2 + 1) * num_buckets), f.read((L2 + 1) * num_buckets * 4)
            )
        )
        ...
        data["l1w"] = list(
            struct.unpack(
                "<" + "b" * (L1 * (L2 + 1) * num_buckets),
                f.read(L1 * (L2 + 1) * num_buckets),
            )
        )

        # Read l2b (32-bit) - 256 values (L3 * 8)
        data["l2b"] = list(
            struct.unpack("<" + "i" * (L3 * num_buckets), f.read(L3 * num_buckets * 4))
        )

        # Read l2w (8-bit) - 7680 values ((L2 + 1) * 2 * L3 * 8)
        data["l2w"] = list(
            struct.unpack(
                "<" + "b" * ((L2 + 1) * 2 * L3 * num_buckets),
                f.read((L2 + 1) * 2 * L3 * num_buckets),
            )
        )

        # Read l3b (32-bit) - 8 values (8)
        data["l3b"] = list(
            struct.unpack("<" + "i" * num_buckets, f.read(num_buckets * 4))
        )

        # Read l3w (8-bit) - 256 values (L3 * 8)
        data["l3w"] = list(
            struct.unpack("<" + "b" * (L3 * num_buckets), f.read(L3 * num_buckets))
        )
...
def organize_into_buckets(data, L1, L2, L3, num_buckets=8):
    ...
    for bucket in range(num_buckets):
        ...
        bucketed_data["l2"].append(
            {"bias": data["l2b"][bias_start:bias_end], "weights": weights}
        )
...
def convert_binary_format(
    input_file, output_file, L1=3072, L2=15, L3=32, inputs=22528, num_buckets=8
):
    ...
    with open(output_file, "wb") as outfile:
        ...
        for bucket in range(num_buckets):
            ...
            outfile.write(
                struct.pack(
                    "<" + "i" * len(bucketed_data["l1"][bucket]["bias"]),
                    *bucketed_data["l1"][bucket]["bias"],
                )
            )
            outfile.write(
                struct.pack(
                    "<" + "b" * len(bucketed_data["l1"][bucket]["weights"]),
            ...
            outfile.write(
                struct.pack(
                    "<" + "i" * len(bucketed_data["l2"][bucket]["bias"]),
                    *bucketed_data["l2"][bucket]["bias"],
                )
            )
            outfile.write(
                struct.pack(
                    "<" + "b" * len(bucketed_data["l2"][bucket]["weights"]),
            ...
            outfile.write(
                struct.pack(
                    "<" + "i" * len(bucketed_data["l3"][bucket]["bias"]),
                    *bucketed_data["l3"][bucket]["bias"],
                )
            )
            outfile.write(
                struct.pack(
                    "<" + "b" * len(bucketed_data["l3"][bucket]["weights"]),
...
def main():
    ...
    parser.add_argument(
        "--L1", type=int, default=3072, help="L1 parameter (default: 3072)"
    )
    ...
    parser.add_argument(
        "--inputs", type=int, default=22528, help="inputs parameter (default: 22528)"
    )
    parser.add_argument(
        "--buckets", type=int, default=8, help="Number of buckets (default: 8)"
    )

    args = parser.parse_args()

    convert_binary_format(
        args.input_file,
        args.output_file,
        args.L1,
        args.L2,
